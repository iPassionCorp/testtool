package th.co.ipassion.testtool.dto.dm;

public class Ordmast {
    private String policyno;
    private String nameid;
    private String branch;
    private String salesid;
    private String occupationtype;
    private String occupationcode;
    private String plancode;
    private String effectivedate;
    private String policystatus1;
    private String policystatus2;
    private String policystatusdate1;
    private String policystatusdate2;
    private String oldpolicystatus1;
    private String oldpolicystatus2;
    private String oldpolicystatusdate1;
    private String oldpolicystatusdate2;
    private String insuredage;
    private String sum;
    private String lifepremium;
    private String mode;
    private String payperiod;
    private String rpno;
    private String paydate;
    private String duedate;
    private String remark;
    private String planpa;
    private String hivflag;
    private String lapseflag;
    private String tabianlapseflag;
    private String medical;
    private String _class;
    private String benefitrate;
    private String contactaddressid;
    private String contactstartdate;
    private String localaddressid;
    private String invalid;
    private String saleschannel;

    public String getPolicyno() {
        return policyno;
    }

    public String getNameid() {
        return nameid;
    }

    public String getBranch() {
        return branch;
    }

    public String getSalesid() {
        return salesid;
    }

    public String getOccupationtype() {
        return occupationtype;
    }

    public String getOccupationcode() {
        return occupationcode;
    }

    public String getPlancode() {
        return plancode;
    }

    public String getEffectivedate() {
        return effectivedate;
    }

    public String getPolicystatus1() {
        return policystatus1;
    }

    public String getPolicystatus2() {
        return policystatus2;
    }

    public String getPolicystatusdate1() {
        return policystatusdate1;
    }

    public String getPolicystatusdate2() {
        return policystatusdate2;
    }

    public String getOldpolicystatus1() {
        return oldpolicystatus1;
    }

    public String getOldpolicystatus2() {
        return oldpolicystatus2;
    }

    public String getOldpolicystatusdate1() {
        return oldpolicystatusdate1;
    }

    public String getOldpolicystatusdate2() {
        return oldpolicystatusdate2;
    }

    public String getInsuredage() {
        return insuredage;
    }

    public String getSum() {
        return sum;
    }

    public String getLifepremium() {
        return lifepremium;
    }

    public String getMode() {
        return mode;
    }

    public String getPayperiod() {
        return payperiod;
    }

    public String getRpno() {
        return rpno;
    }

    public String getPaydate() {
        return paydate;
    }

    public String getDuedate() {
        return duedate;
    }

    public String getRemark() {
        return remark;
    }

    public String getPlanpa() {
        return planpa;
    }

    public String getHivflag() {
        return hivflag;
    }

    public String getLapseflag() {
        return lapseflag;
    }

    public String getTabianlapseflag() {
        return tabianlapseflag;
    }

    public String getMedical() {
        return medical;
    }

    public String get_class() {
        return _class;
    }

    public String getBenefitrate() {
        return benefitrate;
    }

    public String getContactaddressid() {
        return contactaddressid;
    }

    public String getContactstartdate() {
        return contactstartdate;
    }

    public String getLocaladdressid() {
        return localaddressid;
    }

    public String getInvalid() {
        return invalid;
    }

    public String getSaleschannel() {
        return saleschannel;
    }

    public void setPolicyno(String policyno) {
        this.policyno = policyno;
    }

    public void setNameid(String nameid) {
        this.nameid = nameid;
    }

    public void setBranch(String branch) {
        this.branch = branch;
    }

    public void setSalesid(String salesid) {
        this.salesid = salesid;
    }

    public void setOccupationtype(String occupationtype) {
        this.occupationtype = occupationtype;
    }

    public void setOccupationcode(String occupationcode) {
        this.occupationcode = occupationcode;
    }

    public void setPlancode(String plancode) {
        this.plancode = plancode;
    }

    public void setEffectivedate(String effectivedate) {
        this.effectivedate = effectivedate;
    }

    public void setPolicystatus1(String policystatus1) {
        this.policystatus1 = policystatus1;
    }

    public void setPolicystatus2(String policystatus2) {
        this.policystatus2 = policystatus2;
    }

    public void setPolicystatusdate1(String policystatusdate1) {
        this.policystatusdate1 = policystatusdate1;
    }

    public void setPolicystatusdate2(String policystatusdate2) {
        this.policystatusdate2 = policystatusdate2;
    }

    public void setOldpolicystatus1(String oldpolicystatus1) {
        this.oldpolicystatus1 = oldpolicystatus1;
    }

    public void setOldpolicystatus2(String oldpolicystatus2) {
        this.oldpolicystatus2 = oldpolicystatus2;
    }

    public void setOldpolicystatusdate1(String oldpolicystatusdate1) {
        this.oldpolicystatusdate1 = oldpolicystatusdate1;
    }

    public void setOldpolicystatusdate2(String oldpolicystatusdate2) {
        this.oldpolicystatusdate2 = oldpolicystatusdate2;
    }

    public void setInsuredage(String insuredage) {
        this.insuredage = insuredage;
    }

    public void setSum(String sum) {
        this.sum = sum;
    }

    public void setLifepremium(String lifepremium) {
        this.lifepremium = lifepremium;
    }

    public void setMode(String mode) {
        this.mode = mode;
    }

    public void setPayperiod(String payperiod) {
        this.payperiod = payperiod;
    }

    public void setRpno(String rpno) {
        this.rpno = rpno;
    }

    public void setPaydate(String paydate) {
        this.paydate = paydate;
    }

    public void setDuedate(String duedate) {
        this.duedate = duedate;
    }

    public void setRemark(String remark) {
        this.remark = remark;
    }

    public void setPlanpa(String planpa) {
        this.planpa = planpa;
    }

    public void setHivflag(String hivflag) {
        this.hivflag = hivflag;
    }

    public void setLapseflag(String lapseflag) {
        this.lapseflag = lapseflag;
    }

    public void setTabianlapseflag(String tabianlapseflag) {
        this.tabianlapseflag = tabianlapseflag;
    }

    public void setMedical(String medical) {
        this.medical = medical;
    }

    public void set_class(String _class) {
        this._class = _class;
    }

    public void setBenefitrate(String benefitrate) {
        this.benefitrate = benefitrate;
    }

    public void setContactaddressid(String contactaddressid) {
        this.contactaddressid = contactaddressid;
    }

    public void setContactstartdate(String contactstartdate) {
        this.contactstartdate = contactstartdate;
    }

    public void setLocaladdressid(String localaddressid) {
        this.localaddressid = localaddressid;
    }

    public void setInvalid(String invalid) {
        this.invalid = invalid;
    }

    public void setSaleschannel(String saleschannel) {
        this.saleschannel = saleschannel;
    }
}
