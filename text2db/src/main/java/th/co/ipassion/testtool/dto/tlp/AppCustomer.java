package main.java.th.co.ipassion.testtool.dto.tlp;

public class AppCustomer implements java.io.Serializable{
    private String appno;
    private String custprename;
    private String custname;
    private String custsurname;
    private String custstatus;
    private String custheight;
    private String custweight;
    private String custoccupation;
    private String custposition;
    private String custid;
    private String custidtype;
    private String custiddate;
    private String custidamphur;
    private String custidprovince;
    private String custsex;
    private String custage;
    private String importdate;
    private String printstatus;
    private String printdate;
    private String addtype;
    private String custbirthdate;
    private String relation;
    private String accountno;
    private String userimport;
    private String fileimport;
    private String creditno;
    private String credittype;
    private String creditexpire;
    private String address1;
    private String address2;
    private String address3;
    private String address4;
    private String caddress1;
    private String caddress2;
    private String caddress3;
    private String caddress4;
    private String race;
    private String nationality;
    private String prefix;

    public String getAppno() {
        return appno;
    }

    public String getCustprename() {
        return custprename;
    }

    public String getCustname() {
        return custname;
    }

    public String getCustsurname() {
        return custsurname;
    }

    public String getCuststatus() {
        return custstatus;
    }

    public String getCustheight() {
        return custheight;
    }

    public String getCustweight() {
        return custweight;
    }

    public String getCustoccupation() {
        return custoccupation;
    }

    public String getCustposition() {
        return custposition;
    }

    public String getCustid() {
        return custid;
    }

    public String getCustidtype() {
        return custidtype;
    }

    public String getCustiddate() {
        return custiddate;
    }

    public String getCustidamphur() {
        return custidamphur;
    }

    public String getCustidprovince() {
        return custidprovince;
    }

    public String getCustsex() {
        return custsex;
    }

    public String getCustage() {
        return custage;
    }

    public String getImportdate() {
        return importdate;
    }

    public String getPrintstatus() {
        return printstatus;
    }

    public String getPrintdate() {
        return printdate;
    }

    public String getAddtype() {
        return addtype;
    }

    public String getCustbirthdate() {
        return custbirthdate;
    }

    public String getRelation() {
        return relation;
    }

    public String getAccountno() {
        return accountno;
    }

    public String getUserimport() {
        return userimport;
    }

    public String getFileimport() {
        return fileimport;
    }

    public String getCreditno() {
        return creditno;
    }

    public String getCredittype() {
        return credittype;
    }

    public String getCreditexpire() {
        return creditexpire;
    }

    public String getAddress1() {
        return address1;
    }

    public String getAddress2() {
        return address2;
    }

    public String getAddress3() {
        return address3;
    }

    public String getAddress4() {
        return address4;
    }

    public String getCaddress1() {
        return caddress1;
    }

    public String getCaddress2() {
        return caddress2;
    }

    public String getCaddress3() {
        return caddress3;
    }

    public String getCaddress4() {
        return caddress4;
    }

    public String getRace() {
        return race;
    }

    public String getNationality() {
        return nationality;
    }

    public String getPrefix() {
        return prefix;
    }

    public void setAppno(String appno) {
        this.appno = appno;
    }

    public void setCustprename(String custprename) {
        this.custprename = custprename;
    }

    public void setCustname(String custname) {
        this.custname = custname;
    }

    public void setCustsurname(String custsurname) {
        this.custsurname = custsurname;
    }

    public void setCuststatus(String custstatus) {
        this.custstatus = custstatus;
    }

    public void setCustheight(String custheight) {
        this.custheight = custheight;
    }

    public void setCustweight(String custweight) {
        this.custweight = custweight;
    }

    public void setCustoccupation(String custoccupation) {
        this.custoccupation = custoccupation;
    }

    public void setCustposition(String custposition) {
        this.custposition = custposition;
    }

    public void setCustid(String custid) {
        this.custid = custid;
    }

    public void setCustidtype(String custidtype) {
        this.custidtype = custidtype;
    }

    public void setCustiddate(String custiddate) {
        this.custiddate = custiddate;
    }

    public void setCustidamphur(String custidamphur) {
        this.custidamphur = custidamphur;
    }

    public void setCustidprovince(String custidprovince) {
        this.custidprovince = custidprovince;
    }

    public void setCustsex(String custsex) {
        this.custsex = custsex;
    }

    public void setCustage(String custage) {
        this.custage = custage;
    }

    public void setImportdate(String importdate) {
        this.importdate = importdate;
    }

    public void setPrintstatus(String printstatus) {
        this.printstatus = printstatus;
    }

    public void setPrintdate(String printdate) {
        this.printdate = printdate;
    }

    public void setAddtype(String addtype) {
        this.addtype = addtype;
    }

    public void setCustbirthdate(String custbirthdate) {
        this.custbirthdate = custbirthdate;
    }

    public void setRelation(String relation) {
        this.relation = relation;
    }

    public void setAccountno(String accountno) {
        this.accountno = accountno;
    }

    public void setUserimport(String userimport) {
        this.userimport = userimport;
    }

    public void setFileimport(String fileimport) {
        this.fileimport = fileimport;
    }

    public void setCreditno(String creditno) {
        this.creditno = creditno;
    }

    public void setCredittype(String credittype) {
        this.credittype = credittype;
    }

    public void setCreditexpire(String creditexpire) {
        this.creditexpire = creditexpire;
    }

    public void setAddress1(String address1) {
        this.address1 = address1;
    }

    public void setAddress2(String address2) {
        this.address2 = address2;
    }

    public void setAddress3(String address3) {
        this.address3 = address3;
    }

    public void setAddress4(String address4) {
        this.address4 = address4;
    }

    public void setCaddress1(String caddress1) {
        this.caddress1 = caddress1;
    }

    public void setCaddress2(String caddress2) {
        this.caddress2 = caddress2;
    }

    public void setCaddress3(String caddress3) {
        this.caddress3 = caddress3;
    }

    public void setCaddress4(String caddress4) {
        this.caddress4 = caddress4;
    }

    public void setRace(String race) {
        this.race = race;
    }

    public void setNationality(String nationality) {
        this.nationality = nationality;
    }

    public void setPrefix(String prefix) {
        this.prefix = prefix;
    }
}
